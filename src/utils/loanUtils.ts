...
export function generateInstallmentSchedule(
  ...
  const periodicRate = (annualInterestRate / 100) / (frequency === 'Monthly' ? 12 : frequency === 'Bi-Weekly' ? 26 : 52);

  if (method === 'Reducing Balance') {
    // Equated Monthly Installment (EMI) formula
    const emi = periodicRate > 0
      ? (principal * periodicRate * Math.pow(1 + periodicRate, numInstallments)) / (Math.pow(1 + periodicRate, numInstallments) - 1)
    ...
    for (let i = 1; i <= numInstallments; i++) {
      ...
      const interestForPeriod = Math.round(remainingPrincipal * periodicRate * 100) / 100;
      let principalForPeriod = Math.round((emi - interestForPeriod) * 100) / 100;
      ...
      const totalInstallmentAmount = Math.round((principalForPeriod + interestForPeriod) * 100) / 100;
    ...
    // Flat Rate
    const totalInterest = (principal * (annualInterestRate / 100) * (termMonths / 12));
    const flatInterestPerInstallment = Math.round((totalInterest / numInstallments) * 100) / 100;
    const flatPrincipalPerInstallment = Math.round((principal / numInstallments) * 100) / 100;
    const flatTotal = Math.round((flatPrincipalPerInstallment + flatInterestPerInstallment) * 100) / 100;
...
export function calculateEarlySettlementQuote(
  ...
  const outstandingPrincipalBalance = Math.max(0, Math.round((loan.disbursedAmount - principalPaidToDate) * 100) / 100);
  ...
  const accruedInterestToDate = Math.round(outstandingPrincipalBalance * monthlyRate * 100) / 100;
  ...
  const unearnedFutureInterestWaived = Math.max(0, Math.round((totalFutureScheduledInterest - accruedInterestToDate) * 100) / 100);
  ...
  const earlySettlementPenaltyFee = Math.round((outstandingPrincipalBalance * (penaltyPercent / 100)) * 100) / 100;

  // Total required settlement amount
  const totalSettlementAmount = Math.round(
    (outstandingPrincipalBalance + accruedInterestToDate + earlySettlementPenaltyFee) * 100
  ) / 100;
  ...
  const totalSavingsForCustomer = Math.max(0, Math.round((totalContractRemaining - totalSettlementAmount) * 100) / 100);

  return {
    calculationDate: settlementDateStr,
    originalPrincipal: loan.disbursedAmount,
    principalPaidToDate,
