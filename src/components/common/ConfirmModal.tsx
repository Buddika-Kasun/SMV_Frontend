...
export interface ConfirmInputConfig {
  ...
  /** Whether the input is required to enable the confirm button */
  required?: boolean;
  /** Max character length */
  maxLength?: number;
  /** Input type: 'text' | 'textarea' */
  type?: "text" | "textarea";
  /** Number of rows (only for textarea) */
  rows?: number;
...
export interface ConfirmModalProps {
  ...
  isLoading?: boolean;
  details?: { label: string; value: string | number | React.ReactNode }[];
  /** Optional input field (e.g., for rejection reason) */
 ...
 * Used for all critical operations: approvals, rejections, disbursements,
 * payments, early settlements, password updates, and deletions.
 * Optionally supports a text input (e.g., rejection reason).
 */
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  isOpen,
  onClose,
  ...
  // Block confirm if input is required and empty
  ...
  const variantStyles = {
    danger: {
      ...
      borderHeader: "border-rose-100",
      inputFocus: "focus:ring-rose-500/30 focus:border-rose-500",
    },
    warning: {
      icon: <AlertTriangle className="w-6 h-6 text-amber-600" />,
      bgIcon: "bg-amber-100 border-amber-200",
      ...
      borderHeader: "border-amber-100",
      inputFocus: "focus:ring-amber-500/30 focus:border-amber-500",
    },
    info: {
      icon: <Info className="w-6 h-6 text-blue-600" />,
      bgIcon: "bg-blue-100 border-blue-200",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
        {/* Modal Header */}
        <div
          className={`p-5 flex items-start gap-3.5 border-b ${variantStyles.borderHeader}`}
        >
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${variantStyles.bgIcon}`}
          >
            {variantStyles.icon}
          </div>
          <div className="flex-1 pr-2">
          ...
          <div className="p-4 bg-slate-50 border-b border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {details.map((d, idx) => (
                <div
                  key={idx}
                  className="bg-white p-2 rounded-lg border border-slate-200/80"
                >
                  <span className="text-[10px] text-slate-400 block font-medium">
                    {d.label}
                  </span>
          ...
          <div className="p-4 border-b border-slate-100 space-y-1.5">
            ...
            {input.type === "textarea" ? (
              <textarea
                ...
                disabled={isLoading}
                autoFocus
                className={`w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:bg-white focus:ring-2 outline-hidden resize-none transition ${variantStyles.inputFocus}`}
              />
            ) : (
              <input
                type="text"
                ...
                disabled={isLoading}
                autoFocus
                className={`w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:bg-white focus:ring-2 outline-hidden transition ${variantStyles.inputFocus}`}
              />
            )}

            <div className="flex justify-between text-[10px] text-slate-400">
              <span>{input.helperText || ""}</span>
              {input.maxLength && (
                <span>
        ...
        <div className="p-4 bg-slate-50/50 flex items-center justify-end gap-2.5">
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="px-4 py-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition cursor-pointer disabled:bg-gray-400"
          >
            {cancelLabel}
          </button>
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-xl shadow-sm transition flex items-center gap-1.5 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-1 disabled:opacity-50 disabled:bg-gray-400 disabled:cursor-not-allowed ${variantStyles.btnConfirm}`}
          >
            {isLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
            <span>{confirmLabel}</span>
          </button>
