...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const allTabs = [
    ...
    ...(isManagerOrAdmin ? [{
      ...
      badge: overdueCount > 0 ? { count: overdueCount, color: 'bg-amber-100 text-amber-800' } : undefined,
    }] : []),

    /*
    // Consultancy Module - Commented out for now as requested
    {
      id: 'consultancy' as TabType,
      label: 'Consultancy',
      icon: Briefcase,
      badge: activeConsultancyCount > 0 ? { count: activeConsultancyCount, color: 'bg-emerald-100 text-emerald-800' } : undefined,
    },
    */
    ...
    // Operational Modules (Accessible by Staff, Managers, and Admin)
    {
      ...
      icon: FileText,
      badge: pendingApprovalsCount > 0 ? { count: pendingApprovalsCount, color: 'bg-blue-100 text-blue-800' } : undefined,
    },
    {
      id: 'kyc' as TabType,
      label: 'KYC Repository',
      icon: FileCheck,
      badge: pendingKycCount > 0 ? { count: pendingKycCount, color: 'bg-purple-100 text-purple-800' } : undefined,
    },
    {
  ...
  return (
    <aside className="w-full md:w-56 bg-white text-slate-800 flex flex-col shrink-0 border-r border-slate-200/80">
      ...
      <div className="p-4 border-b border-slate-100 flex items-center gap-2.5">
        ...
        <div>
          <span className="font-extrabold text-slate-900 tracking-tight text-sm block">SMV Holdings</span>
          <span className="text-[10px] text-blue-600 font-semibold block">Micro Finance Enterprise</span>
        </div>
      </div>
        ...
        {allTabs.map(tab => {
          ...
          return (
            ...
            >
              ...
              {tab.badge && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${tab.badge.color}`}>
                  {tab.badge.count}
                </span>
              )}
            </button>
      ...
      <div className="p-3 border-t border-slate-100 bg-slate-50/60 text-[10px] text-slate-500 flex items-center justify-between">
        <div className="flex items-center gap-1.5 truncate">
          ...
          <span className="font-medium truncate capitalize">{currentUser.role} Session</span>
        </div>
        <span className="text-[9px] uppercase font-bold text-slate-400">v2.7</span>
      </div>
    </aside>
  );
};
