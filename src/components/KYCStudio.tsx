...
export const KYCStudio: React.FC<KYCStudioProps> = ({
  ...
  const kycLoans = loans.filter(l => l.status === 'KYC Pending' || l.status === 'Approved - Pending Disbursement');
  ...
  const [formData, setFormData] = useState({
    nationalIdNumber: currentLoan?.kyc.nationalIdNumber || '',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Identity verification, document compliance audit, guarantor validation, and loan fund disbursement.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200/80 rounded-xl p-5 shadow-2xs space-y-5">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Applicant Verification</span>
            ...
            <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-medium ${
              ...
              {currentLoan.kyc.isVerified ? 'Verified' : 'Verification Pending'}
            </span>
          </div>

          <form onSubmit={e => { e.preventDefault(); handleSaveKYC(); }} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[11px] font-medium text-slate-600 block mb-1">ID Document Type</label>
                  <select
                    name="idType"
                ...
                <div>
                  <label className="text-[11px] font-medium text-slate-600 block mb-1">ID Number</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[11px] font-medium text-slate-600 block mb-1">Occupation</label>
                  <input
                    type="text"
                    name="occupation"
                ...
                <div>
                  <label className="text-[11px] font-medium text-slate-600 block mb-1">Monthly Income (LKR)</label>
                  <input
                    type="number"
                    name="monthlyIncome"
            ...
            <div className="pt-3 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[11px] font-medium text-slate-600 block mb-1">Guarantor Name</label>
                  <input
                    type="text"
                    name="guarantorName"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200/80 p-5 rounded-xl shadow-2xs space-y-3">
            <div className="flex items-center gap-1.5 text-slate-900">
              ...
              <h3 className="font-semibold text-slate-900 text-xs">Disbursement Action</h3>
            </div>

            <div className="text-xs text-slate-700 space-y-1.5 bg-slate-50/60 p-3 rounded-lg border border-slate-100">
              <div className="flex justify-between">
                <span className="text-slate-500">Loan Amount:</span>
                <span className="font-medium text-slate-900">{formatCurrency(currentLoan.requestedAmount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Fee Deducted:</span>
                <span className="text-slate-700">{formatCurrency(currentLoan.processingFee)}</span>
              </div>
